...
def get_conversational_chain():
    ...
    - If the context is insufficient, say so clearly and write by yourself.
...
def fetch_nitj_today_data():
    """
    Fetches data from https://www.nitj.ac.in/today/ and returns structured event data.
    """
    logger.info("Fetching data from https://www.nitj.ac.in/today/")
    url = "https://www.nitj.ac.in/today/"
    try:
        # Fetch the webpage content
        response = requests.get(url, timeout=10)
        if response.status_code != 200:
            logger.error("Failed to fetch NITJ Today page: Status %d", response.status_code)
            return []

        # Parse HTML content
        soup = BeautifulSoup(response.text, "html.parser")

        # Find the main Container (adjust class as needed based on actual structure)
        main_container = soup.find("div", class_="flex sm:flex-row flex-col items-baseline")
        if not main_container:
            logger.warning("Main container not found on NITJ Today page")
            return []

        # Extract the section title
        section_title = main_container.find("h3")
        section_title_text = section_title.get_text(strip=True) if section_title else "Unknown Section"

        # Find all event entries
        event_containers = main_container.find_all("div", class_="flex flex-row mb-2 gap-1 items-center")
        if not event_containers:
            logger.warning("No event entries found on NITJ Today page")
            return []

        events_data = []
        for event in event_containers:
            # Extract date from the button
            date_button = event.find("button", class_="button--group")
            date_span = date_button.find("span") if date_button else None
            event_date = date_span.get_text(strip=True) if date_span else "Unknown Date"

            # Extract event title/description
            title_span = event.find("span", class_="text-sm text-white px-1 py-1 rounded-1 text-nowrap flex flex-wrap")
            event_title = title_span.get_text(strip=True) if title_span else "Unknown Event"

            # Extract additional info (e.g., duration or scale)
            additional_span = event.find("span", class_="px-1 font-medium")
            additional_info = additional_span.get_text(strip=True) if additional_span else "No Additional Info"
        ...
        logger.info("Successfully fetched %d events from NITJ Today", len(events_data))
        return events_data

    except Exception as e:
        logger.error("Error fetching data from NITJ Today: %s", e)
        return []

def save_events_to_file(events_data, filename="nitj_today_events.json"):
    """
    Saves the fetched events data to a JSON file.
    """
    try:
        with open(filename, "w") as f:
            json.dump(events_data, f, indent=2)
        logger.info("Saved events data to %s", filename)
    except Exception as e:
        logger.error("Error saving events to file: %s", e)

def start_periodic_fetching(interval_minutes=60):
    """
    Starts periodic fetching of data from NITJ Today every 'interval_minutes' minutes.
    Runs in a separate thread to avoid blocking the FastAPI app.
    Returns the Thread object for potential control.
    """
    def job():
        logger.info("Starting periodic fetch job")
        events_data = fetch_nitj_today_data()
        if events_data:
            save_events_to_file(events_data)
        else:
            logger.warning("No events data fetched in this cycle")

    # Schedule the job
    schedule.every(interval_minutes).minutes.do(job)
    logger.info("Scheduled periodic fetching every %d minutes", interval_minutes)

    # Initial fetch
    job()

    # Define the loop for the scheduler
    def run_scheduler():
        while True:
    ...
    fetch_thread = Thread(target=run_scheduler, daemon=True)
    fetch_thread.start()
    logger.info("Started periodic fetching thread")
    return fetch_thread

# --------------------------- SerpAPI Search Function ---------------------------

def search_google(query, api_key, num_results=2):
    logger.info("Searching Google via SerpAPI for query: %s", query)
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error("SerpAPI request failed with status: %d", response.status_code)
            return []
        ...
        logger.debug("Extracted %d results from SerpAPI", len(extracted))
        return extracted
    except Exception as e:
        logger.error("Error in SerpAPI search: %s", e)
        return []

# --------------------------- Content Extraction from Links ---------------------------

def extract_all_text(text):
    ...
    def recursive_text_extract(element):
        ...
        if isinstance(element, NavigableString):
            ...
            for child in element.children:
                texts.extend(recursive_text_extract(child))
        return texts

    all_texts = recursive_text_extract(soup)
...
def extract_text_from_url(url):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error("Failed to fetch URL %s: Status %d", url, response.status_code)
            return ""
        ...
        if "text/html" in content_type:
            cleaned = extract_all_text(response.text)
            logger.debug("Extracted HTML text, length: %d", len(cleaned))
            return cleaned
        ...
        # Handle PDF content
            ...
            try:
                ...
                for i, image in enumerate(images):
                    ...
                    pdf_text += text + "\n"
                    logger.debug("Extracted text from PDF page %d", i + 1)
                ...
                logger.debug("Extracted PDF text, length: %d", len(cleaned))
                return cleaned
            finally:
        ...
        # Handle plain text
            ...
            logger.debug("Extracted plain text, length: %d", len(cleaned))
            return cleaned
        
        else:
            logger.warning("Unsupported content type for URL %s: %s", url, content_type)
            return ""
    except Exception as e:
        logger.error("Error extracting content from URL %s: %s", url, e)
        return ""

def build_context_from_links(search_results):
    logger.info("Building context from %d search results", len(search_results))
    context = ""
    ...
    logger.debug("Built context, length: %d characters", len(context))
    return context

# --------------------------- Gemini Response Function ---------------------------

def get_gemini_response(context, question):
    logger.info("Querying Gemini with context and question: %s", question[:50])
    try:
        model = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0.3)
        prompt_template = """
        You are an intelligent assistant answering questions based strictly on the given context. Use the information provided to answer comprehensively, accurately, and in the appropriate format.
        ...
        - Use the given context to answer. If the answer is not found, respond: "Answer is not available in the context."
        - Never fabricate or assume facts not in the context.
        - If a question involves steps, procedures, or processes, use clear bullet points.
        - For definitions or factual queries: provide concise, formal answers.
        - For lists: use bullet points.
        - For how-to or process questions: step-by-step format.
        - For comparisons: use tables or summaries.
        - If the context is insufficient, say so clearly,write by yourself.
        - Use **bold headings**, bullet points, and numbered lists where applicable.
        - Structure your answers for readability.
        - Use tables if comparisons are involved.
        ...
        logger.debug("Gemini response generated: %s", response_text[:50])
        return response_text
    except Exception as e:
        logger.error("Error querying Gemini: %s", e)
...
async def ask_question(data: dict):
    ...
    try:
        ...
        # Check if response indicates answer not in context
        if "Answer is not available in the context" in response_text and not offline_mode:
            logger.info("Answer not in context, initiating SerpAPI search")
            ...
            search_results = search_google(user_question, api_key, num_results=2)
            if not search_results:
                logger.warning("No search results from SerpAPI")
                return {"response": "Answer is not available in the context and no search results found."}
            
            # Build context from search results
            context = build_context_from_links(search_results)
            if not context:
                logger.warning("No content extracted from search results")
                return {"response": "Answer is not available in the context and no content could be extracted from search results."}
            
            # Query Gemini with the new context
            documents = [Document(page_content=context)]
            response_text = get_gemini_response(documents, user_question)
...
if __name__ == "__main__":
    # Start periodic fetching before the server starts
    start_periodic_fetching(interval_minutes=60)
